...
class Application():
    ...
    # Utilisée pour mémoriser les pièces au mauvais emplacement
    wrong_pos = list()
            ...
            if k != self.authorized_pos[k].ob:
                ...
                if self.authorized_pos[k].ob is not None:
                    ...
                    rectangle = self.cnv.create_rectangle(x, y, x +
                                                          ...
                                                          width=2,
                                                          stipple="gray50")
                    self.wrong_pos.append([self.authorized_pos[k].ob,
                                           rectangle])
        if not self.victory:
    ...
    def final_pos(self, x, y):
        ...
        if valid:
            ...
            if place is not None:
                # Si le clic est effectué sur le même objet,
                # on le retourne dans le deck
